...
namespace AFI.Domain.Registration
{
    public class Customer : ICustomer
    {
            ...
            string? email)
        {
            ...
            {
                throw new ValidationException("First name must be between 3 and 50 characters long.");
            }

            if (string.IsNullOrWhiteSpace(surname) || surname.Length < 3 || surname.Length > 50)
            {
                throw new ValidationException("Surname must be between 3 and 50 characters long.");
            }
            ...
            {
                throw new ValidationException("Date of Birth or Email required.");
            }

            DateTime eighteenYearsFromToday = DateTime.Today.AddYears(-18);
            if (dateOfBirth != null && !IsOver18(dateOfBirth))
            {
                throw new ValidationException("Policy holder must be 18 or older.");
            }

            if (!string.IsNullOrWhiteSpace(email) && !Regex.IsMatch(email, RegexPatterns.EmailAddressPattern))
            {
                throw new ValidationException("Email does not match the expected email format.");
            }
